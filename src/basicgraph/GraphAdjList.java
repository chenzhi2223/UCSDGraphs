...
public class GraphAdjList extends Graph {
	 ...
	 */		
	 public List<Integer> getDistance2(int v) {
		 ...
		 if (v>=getNumVertices()) return null;
		 else {
			 List<Integer> list= new ArrayList<Integer>();	
			 //1-hop neighbor list from vertex v;
		     List<Integer> list2 = new ArrayList<Integer>(adjListsMap.get(v));
		     for (int w: list2) {
		     //check for 2-hop reachable vertices.
		    	 List<Integer> list3 = new ArrayList<Integer>(adjListsMap.get(w));
		    	 for (int u: list3) { 
		    		list.add(u);
		    	 }
			 }
